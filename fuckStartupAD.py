...
def main():
    ...
    photos = [f for f in os.listdir(photo_dir) if f.lower().endswith('.jpg') or f.lower().endswith('.jpeg')]
    if not photos:
        print("照片目录下没有 jpeg 图片。")
        return

    # 3. 遍历 startupImage 下的所有子文件夹
    subfolders = [os.path.join(startup_image_dir, d) for d in os.listdir(startup_image_dir) if os.path.isdir(os.path.join(startup_image_dir, d))]
    ...
    for folder in subfolders:
        ...
        for i, new_name in enumerate(rename_list):
            ...
            shutil.copy(os.path.join(photo_dir, photo_file), os.path.join(folder, new_name))
